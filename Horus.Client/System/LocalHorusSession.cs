...
namespace Horus.Client.System
    ...
    internal class LocalHorusSession : HorusSession
        {
        List<LocalHorusDriver> allLocalDrivers = null;

        void EnsureLocalDrivers()
            {
        ...
        TInterface CreateDriverInstance<TInterface>(LocalHorusDriver localDriver) where TInterface : class
            {
            // HACK: Drivers should be loaded into separate AppDomains
            if (
                AppDomain.CurrentDomain.GetAssemblies()
                         .SingleOrDefault(x => x.FullName == localDriver.Assembly.GetName().FullName) == null)
                {
        ...
        public override HorusDriverSummary[] EnumDrivers()
            ...
            foreach (LocalHorusDriver localDriver in allLocalDrivers)
                ...
                Type[] implementedHorusInterfaces =
                    localDriver.Implementor.FindInterfaces(
                        (type, criteria) => typeof(IHorusDriver).IsAssignableFrom(type), null);
                rv.Add(
                    new HorusDriverSummary()
                        {
